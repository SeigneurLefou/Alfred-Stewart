...
def write_json_file(dict_face):
    json.dump(dict_face, open("{}media/face.json".format(jsonvar["userfolder"]), 'w', encoding='utf-8'), indent=4, ensure_ascii=False)

def add_face_json_with_str(emotion, str_face):
    with open("{}media/face.json".format(jsonvar["userfolder"]), "r", encoding='utf-8') as face_json:
